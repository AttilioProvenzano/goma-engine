...
result<Shader*> Device::CreateShader(ShaderDesc shader_desc) {
    // Try to retrieve the SPIR-V from cache
    struct ShaderSourceHasher {
        size_t operator()(const ShaderDesc& desc) const {
            size_t seed = 0;
            hash_combine(seed, desc.stage);
            hash_combine(seed, djb2_hash(desc.source.c_str()));
            hash_combine(seed, djb2_hash(desc.preamble.c_str()));
            return seed;
        }
    };

    std::stringstream filename;
    filename << GOMA_CACHE_DIR "shaders/" << ShaderSourceHasher{}(shader_desc)
             << ".spv";

    auto cached_spirv = Platform::ReadFile(filename.str(), true);
    std::vector<uint32_t> spirv;

    if (cached_spirv) {
        spirv.resize(cached_spirv.value().size() / sizeof(spirv[0]));
        memcpy(spirv.data(), cached_spirv.value().c_str(),
        ...
        shader.setEnvInput(glslang::EShSourceGlsl,
                           stage_map.at(shader_desc.stage),
        ...
        bool compilation_res = shader.parse(
            &glslang::DefaultTBuiltInResource, 100, false,
    ...
    // Cache the SPIR-V output
    auto _ =
        Platform::WriteFile(filename.str(), spirv.size() * sizeof(spirv[0]),
                            reinterpret_cast<const char*>(spirv.data()), true);
